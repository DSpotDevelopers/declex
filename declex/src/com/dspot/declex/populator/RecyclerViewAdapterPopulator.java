...
class RecyclerViewAdapterPopulator extends BaseClassPlugin {
	...
	protected static final Logger LOGGER = LoggerFactory.getLogger(RecyclerViewAdapterPopulator.class);
	...
	public RecyclerViewAdapterPopulator(PopulatorHandler handler, String fieldName, String adapterClassName, String modelClassName, 
		...
		annotationHelper = new TargetAnnotationHelper(environment, handler.getTarget());
	}
	...
	public void process(Element element, JDefinedClass AdapterClass) {
		...
		JMethod onCreateViewHolder = AdapterClass.getMethod("onCreateViewHolder", new AbstractJType[]{getClasses().VIEW_GROUP, getCodeModel().INT});
		JFieldRef parent = ref("parent");
		JFieldRef position = ref("position");
		JFieldRef inflater = ref("inflater");
		JBlock onCreateViewMethodBody = onCreateViewHolder.body();
...
//			LOGGER.warn("\nPopulator layouts: " + layoutObjects, element, element.getAnnotation(Populator.class));
		...
		JVar rootView = onCreateViewMethodBody.decl(
				getClasses().VIEW, 
				"rootView",
				inflater.invoke("inflate").arg(contentViewId).arg(_null())
			);
		...
		if (modelClassName.equals(String.class.getCanonicalName())) {
			...
			if (viewClass != null) {		
				...
				onCreateViewMethodBody.assign(viewHolder.ref(viewField), findViewById);
		...
		JMethod onBindViewHolder = AdapterClass.getMethod(
				"onBindViewHolder", 
				new AbstractJType[]{getJClass(viewHolderClassName), getCodeModel().INT}
			);
		...
		JVar model = onBindMethodBody.decl(JMod.FINAL, Model, "model", modelAssigner);
		...
		if (modelClassName.equals(String.class.getCanonicalName())) {
			...
			if (viewClass != null) {
				...
				handler.putAssignInBlock(
					info, onBindMethodBody, view, model, element, viewsHolder, 
		...
		handler.callPopulatorMethod(fieldName, onBindMethodBody, viewHolder, fieldNames, element, viewsHolder);	
